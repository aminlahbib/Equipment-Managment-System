...
export const Navbar: React.FC<NavbarProps> = ({ currentPage, onNavigate, isDarkMode, toggleTheme }) => {
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => onNavigate('dashboard')}>
            <Package size={22} className="text-text-primary transition-transform group-hover:scale-110" />
          ...
          <div className="hidden md:flex items-center space-x-6">
            <div className="relative group">
               <Search size={16} className="text-text-secondary absolute left-0 top-1/2 -translate-y-1/2 transition-colors group-focus-within:text-text-primary" />
               <input 
                 type="text" 
                 placeholder="Search" 
            ...
            <div className="flex items-center gap-4 border-l border-border pl-6">
              ...
              >
                <img src={CURRENT_USER.avatar} alt="User" className="w-8 h-8 rounded-full bg-surfaceHighlight object-cover" />
              </button>
            </div>
          </div>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-surface border-b border-border animate-fade-in-down">
          <div className="px-4 pt-2 pb-6 space-y-2">
             <button onClick={() => onNavigate('dashboard')} className="block w-full text-left py-3 text-text-primary border-b border-border">Overview</button>
             <button onClick={() => onNavigate('equipment')} className="block w-full text-left py-3 text-text-secondary border-b border-border">Inventory</button>
             <button onClick={() => onNavigate('loans')} className="block w-full text-left py-3 text-text-secondary border-b border-border">Activity</button>
             <button onClick={() => onNavigate('landing')} className="block w-full text-left py-3 text-danger mt-2">Sign Out</button>
          </div>
        </div>
      )}
    </nav>
